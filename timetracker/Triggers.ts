...
function onFormSubmit(e) {
    ...
    FormApp.openById("15DrDtgMquviAVpbfg9LDG6pY3OKVi01D9sL48KCdHdM").deleteAllResponses();
}
function onOpen() {
    SpreadsheetApp.setActiveSheet(SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CurrentTrackers"));
    var range = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("GetTrackersForm").getRange("E2:E");
...
function installableOnEdit(e) {
    ...
    var test = curSheet == "CurrentTrackers" &&
        ...
        oldValue !== undefined &&
        newValue === undefined;
    Logger.log("\nsheet: %s\nrow: %s (%s selected)\ncolumn: %s (%s selected)\n bool: %s", curSheet, curRow, numRows, curColumn, numColumns, test);
    if (test) {
