...
function addToTracker() {
    ...
    var response = ui.prompt("Enter how much you want to add to the tracker " + tracker.name +
    ...
    loggerSheet.getRange(tracker.row, 2, 1, values[0].length).setValues(values).setNumberFormats(formats);
    return;
}
function EpochToUTC(epochS) {
    return new Date(epochS * 1000); // Convert to milliseconds
}
