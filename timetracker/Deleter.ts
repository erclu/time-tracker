...
function deleteTracker(trackerName, logsRow) {
    ...
    var logsSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CurrentTrackers");
    var trackersForm = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("GetTrackersForm");
    var deletedEvents = 0;
    for (var currentRow = trackersForm.getLastRow(); currentRow > 0; currentRow--) {
