...
function logEntry(row) {
    ...
    ss.getSheetByName("GetTrackersForm").getRange(row, 5, 1, 1).setValue(true).setFontFamily("Roboto Mono");
    return;
}
;
function logAll() {
    var rows = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("GetTrackersForm").getLastRow();
    Logger.log(rows);
...
function findMatchingTrackerRow(name) {
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CurrentTrackers");
